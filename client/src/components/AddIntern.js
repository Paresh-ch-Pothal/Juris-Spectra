...
const AddIntern = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const response = await fetch(`https://juris-spectra.vercel.app/api/addintern`, {
            ...
            body: JSON.stringify({ title: formData.title, author: formData.author, homedesc: formData.homedesc, internimg: formData.imglink, desc: sections })
        })
        ...
        <>
            {auth ? (
                <div className="container">
                    <h1 className='text-center'>Add Intern News</h1>
                    <ToastContainer
                        position="top-left"
                        autoClose={1500}
                        hideProgressBar={false}
